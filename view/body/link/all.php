<div id="page" class="container">
	<div id="content">
		<div class="sub-content show">
			<div class="post">
                <?=$route->createBreadcrumbs(' > ');?>
                <h2 class="ui dividing header">All Links</h2>
				<div class="post_cell">
					<div class="ui segment">
					  <div class="ui stackable three column very relaxed grid">
						<div class="column">
							<div class="ui list">
							    <div class="item">
								    <div class="ui medium header">駭客常用工具</div>
							    </div>
							    <a href="https://sdc-iss.tainan.gov.tw:4000" class="item" target="_blank">OpenVas(開源掃描工具)</a>
							    <a href="https://hackertarget.com" class="item" target="_blank">Hacker 線上工具</a>
							    <a href="https://hackertarget.com/reverse-ip-lookup" class="item" target="_blank">Reverse IP Lookup</a>
							    <a href="https://mxtoolbox.com/DNSLookup.aspx" class="item" target="_blank">Online DNS Query</a>
							    <a href="https://www.ssllabs.com/ssltest" class="item" target="_blank">Online SSL Test</a>
							    <a href="https://www.abuseipdb.com" class="item" target="_blank">Online AbuseIPDB</a>
							    <a href="https://www.shodan.io" class="item" target="_blank">Shodan Search Engine</a>
							</div>
					    </div>
						<div class="column">
							<div class="ui list">
							    <div class="item">
								    <div class="ui medium header">中心相關系統</div>
							    </div>
							    <a href="http://10.3.40.21:5601/app/kibana#/discover?_g=()" class="item" target="_blank">Elastic Kibana</a>
							    <a href="https://advantage.mandiant.com" class="item" target="_blank">Mandiant</a>
						    </div>
						</div>
						<div class="column">
							<div class="ui list">
							    <div class="item">
								    <div class="ui medium header">行政院相關系統</div>
							    </div>
							    <a href="https://www.ncert.nat.gov.tw" class="item" target="_blank">國家資通安全通報應變網站</a>
							    <a href="https://www.ncert.nat.gov.tw/iAuth2" class="item" target="_blank">資安人員身分驗證系統</a>
							    <a href="https://spm.nat.gov.tw" class="item" target="_blank">資通安全署資通安全作業管考系統</a>
							    <a href="https://ctts.nccst.nat.gov.tw" class="item" target="_blank">資安人才培訓服務網</a>
							    <a href="https://vans.nccst.nat.gov.tw" class="item" target="_blank">資通安全弱點通報系統(VANS)</a>
							    <a href="https://isg.nccst.nat.gov.tw" class="item" target="_blank">資安治理成熟度評估系統(ISG)</a>
						    </div>
						</div>
					  </div>
					 <!-- <div class="ui vertical divider">
						and
					  </div>-->
					</div>
				</div> <!--end of .post_cell -->
			</div> <!--end of .post --> 
		</div>
		<div style="clear: both;">&nbsp;</div>
	</div><!-- end #content -->
</div> <!--end #page-->
